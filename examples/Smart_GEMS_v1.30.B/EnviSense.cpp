...
void EnviSense::AtlasEZO(uint8_t address, const char *cmd, uint16_t time) {
  ...
  if (strcmp(cmd, "sleep") != 0) {  // if the command that has been sent is NOT the sleep command, wait the correct amount of time and request data
    ...
    switch (code) {                        // switch case based on what the response code is
      ...
      case 2:                             // decimal 2
        debug(F("debug print: Failed"));  // means the command has failed
        debug(F("\n"));
        this->ezo_stat = SYNTAX_ERROR;     // assign status to variable
        break;                             // exits the switch case
      case 254:                            // decimal 254
        debug(F("debug print: Pending"));  // means the command has not yet been finished calculating
        debug(F("\n"));
        this->ezo_stat = NOT_READY;        // assign status to variable
        break;                             // exits the switch case
      case 255:                            // decimal 255
        debug(F("debug print: No Data"));  // means there is no further data to send
        debug(F("\n"));
...
void EnviSense::gprsGoogleSheet(String google_script_id, String payload) {
  ...
  // Create HTTPS URL e.g. https://script.google.com/macros/s/AKfycbx78mYXnuL5ygu8A3ORZnl9TWeccnddMbbiRKGVZXiP0h4RHEpY/exec?id=ENVTMP&action=addItem&ph=777&ph2=777&ec=777&tds=777&sal=777&swl=777&temp=777&battlvl=777
