...
#define DASHBOARD "https://script.google.com/macros/s/AKfycbx78mYXnuL5ygu8A3ORZnl9TWeccnddMbbiRKGVZXiP0h4RHEpY/exec?id=ENVTMP&action=getItems"
...
unsigned long SENDING_INTERVAL_S = 900;     // default sending interval every 15 minutes (default)
...
byte deviceMode = 2;        // store device mode status 1: calibration, 2: reading & transmission (default)
...
String DEVICE_ID = "ENVTMP";                                                             // define device ID id of the sensor unit
...
void loop() {
  ...
  if (event) {
    if (computerData == "envs -help\r") {
      ...
      Serial.print("\n");
      Serial.print(F("verbose: envs mode-deploy     // Set device mode for reading & data transmission"));
      Serial.print("\n");
      Serial.print(F("verbose: ***************************************************************************"));
      Serial.print("\n");
    ...
    } else if (computerData == "envs sendevery-5\r") {
      SENDING_INTERVAL_S = 300;
      Serial.print(F("\n"));
      Serial.print(F("verbose: sending interval every 5 mins OK."));
      Serial.print(F("\n"));
    } else if (computerData == "envs sendevery-10\r") {
      SENDING_INTERVAL_S = 600;
      Serial.print(F("\n"));
      Serial.print(F("verbose: sending interval every 10 mins OK."));
      Serial.print(F("\n"));
      ...
      Serial.print(F("\n"));
    } else if (computerData == "envs sendevery-20\r") {
      SENDING_INTERVAL_S = 1200;
      Serial.print(F("\n"));
      Serial.print(F("verbose: sending interval every 20 mins OK."));
      Serial.print(F("\n"));
    } else if (computerData == "envs sendevery-25\r") {
      SENDING_INTERVAL_S = 1500;
      Serial.print(F("\n"));
      Serial.print(F("verbose: sending interval every 25 mins OK."));
      Serial.print(F("\n"));
