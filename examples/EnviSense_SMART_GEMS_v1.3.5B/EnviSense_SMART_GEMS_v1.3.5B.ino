...
void loop() {
      ...
      if (deviceMode == 1) {
        switch (PH_PROBE_TYPE) {
          case 1:
            ...
            if (PH_PROBE_TYPE == 1) UC.calibrateGravitypH(adc_milliVolts_pH, 25.0, "calph");  // calibrate pH at low-point (pH 4)
            break;
          case 2:
      ...
      if (deviceMode == 1) {
        switch (PH_PROBE_TYPE) {
          case 1:
            adc_milliVolts_pH = analogRead(PH_ADC_PIN) / 1023.0 * 5000.0;                     // read the voltage
            ph_val = UC.getGravitypH(adc_milliVolts_pH, 25.0);                                // convert voltage to pH with temperature compensation @ 25°C
            if (PH_PROBE_TYPE == 1) UC.calibrateGravitypH(adc_milliVolts_pH, 25.0, "calph");  // calibrate pH at mid-point (pH 7)
            break;
          case 2:
  ...
  if (deviceMode == 2) {
    if (CURRENT_TIME_S - PREV_SENDING_INTERVAL_S >= SENDING_INTERVAL_S) {
      ...
      // get local time
      String time = UC.gprsGetLocalTime();
